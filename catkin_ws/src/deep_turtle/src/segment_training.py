...
from tensorflow.keras.layers import Input, Convolution2D, Flatten, Dense, Dropout, Dense
...
from data_generator_segment import SegmentDataGenerator
...
def get_model(img_size):
    img_in = Input(shape=(img_size[1], img_size[0], 1), name='img_in')
    x = img_in
...
if __name__ == '__main__':
    # User Options
    # img size is correct this way, I checked it
    gen_params = {'img_size': (128, 96),
                  'batch_size': 16,
                  'channels': 'rgb',
                  'shuffle': True}
    load_model = False
    ...
    training_generator = SegmentDataGenerator(train_dir_paths, **gen_params)
    val_dir_paths = [os.path.join(ws_root, 'data', folder) for folder in val_folders]
    val_generator = SegmentDataGenerator(val_dir_paths, **gen_params)
